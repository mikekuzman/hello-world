...
- **Red glow/border** = Eastern hemisphere (x > 0)
- **Blue glow/border** = Western hemisphere (x < 0)
...
- Color shows location on the 4D hypersphere (eastern vs western hemisphere)
...
The visualizer expects JSON files in `snapshot_set_v005` format:
...
- [ ] **Global subsampling parameter**: Downsample points for better performance
...
**"Unsupported format" error**: Make sure you're using v005 JSON files from `sim_v005.py`

**Black screen**: Check browser console for errors, ensure WebGL is enabled

**Slow performance**: Try smaller simulations (reduce N parameter in sim_v005.py) or wait for v2 subsampling
